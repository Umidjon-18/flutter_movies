...
class DetailViewModel extends ChangeNotifier {
  ...
  uploadMovieDetails(String movieId) async {
    if (await NetworkConnection.checkConnection()) {
      ...
      var productionCompanies = List.generate(
          request['production_companies'].length,
          (index) => ProductionCompanies.fromJson(
              request['production_companies'][index]));
      var productionCountries = List.generate(
          request['production_countries'].length,
          (index) => ProductionCountries.fromJson(
              request['production_countries'][index]));
      var spokenLanguages = List.generate(
          request['spoken_languages'].length,
          (index) =>
              SpokenLanguages.fromJson(request['spoken_languages'][index]));
